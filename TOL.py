...
def generate_subkeys(key_bits):
    ...
    for round_num, shift in enumerate(shifts, 1):
        ...
        # Menampilkan hasil pergeseran dan hasil permutasi PC-2
        print(f"\nC{round_num}: {' '.join(map(str, C))}")
        print(f"D{round_num}: {' '.join(map(str, D))}")

        print(f"Subkunci K{round_num} setelah Permutasi PC-2:")
        for i in range(0, len(subkey), 8):  # Menampilkan dalam blok 8 bit
            print(' '.join(map(str, subkey[i:i+8])))
